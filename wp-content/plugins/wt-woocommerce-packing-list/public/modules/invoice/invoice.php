...
class Wf_Woocommerce_Packing_List_Invoice
{
	...
	public function __construct()
	{
		...
		if($this->is_enable_invoice=='Yes') /* `print_it` method also have the same checking */
		{
			...
			add_filter('wt_email_print_actions',array($this,'add_email_print_buttons'), 10, 5);
		...
		*/
		...
		/**
		* @since 4.0.9 Add to cloud printing
		*/
		add_filter('wt_pklist_add_to_cloud_printing', array($this, 'add_to_cloud_printing'), 10, 1);
		
		/**
		* @since 4.0.9 Do cloud printing
		*/
		add_filter('wt_pklist_do_cloud_printing', array($this, 'do_cloud_printing'), 10, 2);
	}

	/**
	*	@since 4.0.9
	*	Add to cloud printing, this will enable cloud printing settings
	*/
	public function add_to_cloud_printing($arr)
	{
	...
	*	@since 4.0.9
	*	Do cloud printing. 
	*	Checks the current module needes a cloud printing now 
	*/
	public function do_cloud_printing($module_base_arr, $order_id)
	{
		if(!is_null($this->customizer) && isset($module_base_arr[$this->module_base]))
        { 
        	$order_ids=array($order_id);
        	$pdf_name=$this->customizer->generate_pdf_name($this->module_base, $order_ids);
        	$module_base_arr[$this->module_base]=array(
	...
	*/
	public function alter_customize_info_text($info_text,$type,$template_type)
	{
		if($template_type==$this->module_base)
		{
			...
			{
				$info_text=__('You can control the visibility of the seal according to order status via filters. See filter documentation','wf-woocommerce-packing-list').' <a href="'.admin_url('admin.php?page=wf_woocommerce_packing_list#wf-help#filters').'" target="_blank">here</a>.';
			}
		}
		return $info_text;
	}
	...
	*/
	...
	public function add_email_print_buttons($html,$order,$order_id, $email_obj, $sent_to_admin)
	{
		return $this->add_print_button($html, $order, $order_id, true, $sent_to_admin);
	}
	public function add_frontend_print_buttons($html, $order,$order_id)
	{
		return $this->add_print_button($html, $order, $order_id);
	}

	public function add_print_button($html, $order, $order_id, $email_button=false, $sent_to_admin=false)
	{
		...
		if($show_on_frontend=='Yes')
		{
	        ...
	        {
	            Wf_Woocommerce_Packing_List::generate_print_button_for_user($order,$order_id, $this->module_base, esc_html__('Print Invoice', 'wf-woocommerce-packing-list'), $email_button, $sent_to_admin); 
	        }
	    }
	    return $html;
	}
