...
class Wf_Woocommerce_Packing_List_Invoice
{
	...
	*/
	public function alter_customize_info_text($info_text,$type,$template_type)
	{
		if($template_type==$this->module_base)
		{
			...
			{
				$info_text=__('You can control the visibility of the seal according to order status via filters. See filter documentation','print-invoices-packing-slip-labels-for-woocommerce').' <a href="'.admin_url('admin.php?page=wf_woocommerce_packing_list#wf-help#filters').'" target="_blank">here</a>.';
			}
		}
		return $info_text;
	}
	...
	*/
	...
	public function validate_settings_data($val,$key)
	{
		switch ($key) 
		{
			...
			case 'woocommerce_wf_add_invoice_in_mail':
			case 'woocommerce_wf_packinglist_frontend_info':
			case 'woocommerce_wf_invoice_number_format':
			case 'woocommerce_wf_invoice_number_prefix':
			case 'woocommerce_wf_invoice_number_postfix':
			case 'woocommerce_wf_invoice_as_ordernumber':
			case 'woocommerce_wf_enable_invoice':
			case 'woocommerce_wf_add_customer_note_in_invoice':
			case 'woocommerce_wf_packinglist_variation_data':
			case 'woocommerce_wf_orderdate_as_invoicedate':
				$val=sanitize_text_field($val);
			break;
			case 'wf_'.$this->module_base.'_contactno_email':
			case 'woocommerce_wf_attach_invoice':
			case 'woocommerce_wf_generate_for_orderstatus':
				$val=array_map('sanitize_text_field',$val);
			break;
			case 'woocommerce_wf_Current_Invoice_number':
			case 'woocommerce_wf_invoice_start_number':
			case 'woocommerce_wf_invoice_padding_number':
				$val=intval($val);
			break;
			default:
	...
	*/
    public static function get_invoice_date($order_id,$date_format,$order)
    {
    	...
    	if($invoice_date)
    	{
    		return (empty($invoice_date) ? '' : date($date_format,$invoice_date));
    	}else
    	{
	...
	*/
	...
	public function get_customizable_items($settings,$base_id)
	{
		if($base_id==$this->module_id)
		{
			...
			return array(
				'company_logo'=>__('Company Logo','print-invoices-packing-slip-labels-for-woocommerce'),
				//'barcode_disabled'=>__('Bar Code','print-invoices-packing-slip-labels-for-woocommerce').$only_pro_html,
				...
				'product_table_subtotal_disabled'=>__('Sub Total','print-invoices-packing-slip-labels-for-woocommerce').$only_pro_html,
