...
class Wf_Woocommerce_Packing_List_Shippinglabel
{
	...
	public function get_customizable_items($settings,$base_id)
	{
		if($base_id==$this->module_id)
		{
			...
			return array(
				...
				'barcode'=>__('Bar Code','print-invoices-packing-slip-labels-for-woocommerce'),
				//'footer'=>__('Footer','print-invoices-packing-slip-labels-for-woocommerce'),
				'return_policy'=>__('Return Policy','print-invoices-packing-slip-labels-for-woocommerce').$only_pro_html,
			);
		}
		return $settings;
	}
	...
	*/
	...
	{	
		switch ($key) 
		{
			case 'woocommerce_wf_enable_multiple_shipping_label':
			case 'woocommerce_wf_packinglist_footer_sl':
				$val=sanitize_text_field($val);
			break;
			case 'wf_'.$this->module_base.'_contactno_email':
				$val=array_map('sanitize_text_field',$val);
			break;
			case 'woocommerce_wf_packinglist_label_size':
			case 'wf_shipping_label_column_number':
				$val=intval($val);
			break;
			default:
