...
class Wf_Woocommerce_Packing_List {
    ...
    */
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path(dirname( __FILE__ )).'/admin/wf_api_manager/wf-api-manager-config.php';
	...
	*/
	private function define_common_hooks()
	{
		...
		$this->loader->add_filter('locale', $this, 'switch_locale', 10);
	...
	*/
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action('wp_ajax_wt_pklist_checkout_field_list_view',$this->plugin_admin,'checkout_field_list_view');
		...
		*/
		...
		$this->plugin_admin->register_tooltips();
	...
	*/
	...
	public static function load_modules($module_id)
	{
		if(Wf_Woocommerce_Packing_List_Public::module_exists($module_id) || Wf_Woocommerce_Packing_List_Admin::module_exists($module_id))
		{
			...
			{
				$class_name=str_replace(' ','_',ucwords(str_replace('-',' ', $module_id)));
    ...
    */
    public static function generate_print_button_for_user($order, $order_id, $template_type, $label, $email_button=false, $sent_to_admin=false)
    {
    	$show_button=true;
    	
    	/* toggle the visibility for admin/customer */
    	$show_button=apply_filters('wt_pklist_toggle_email_print_buttons', $show_button, $order, $template_type, $email_button, $sent_to_admin);

    	if($show_button)
    	{
    		$action='print_'.$template_type;
